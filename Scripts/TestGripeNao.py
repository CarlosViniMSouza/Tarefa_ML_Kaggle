...
for i in range(0, len(tabela['Calafrios'])):
    ...
    if (tabela['Calafrios'][i] == "n" and tabela["GRIPE"][i] == "n"):
        quantCalafrios += 1
...
for i in range(0, len(tabela['Cefaleia'])):
    ...
    if (tabela['Cefaleia'][i] == "n" and tabela["GRIPE"][i] == "n"):
        quantCefaleia += 1
...
# Aplicando a Formula
ProbGripeNao = ((quantCalafrios/quantGripeNao) * (quantCefaleia/quantGripeNao) *
                (quantCoriza/quantGripeNao) * (quantFebre/quantGripeNao))/(quantGripeNao/len(tabela['GRIPE']))

print("O resultado de P(x|GRIPE = NAO) eh: ", ProbGripeNao, "\n")
